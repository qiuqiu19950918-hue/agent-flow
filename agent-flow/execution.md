...
执行类的回报按**蓝图强度**分流：
- **精确蓝图**（code-executor，主 Agent 已给 old/new）→ 成功只回**状态 + 产物指针**（如"已改完 3/3"）。做了什么蓝图已含，不回。
- **语义蓝图**（general-purpose 动态开发，主 Agent 只给目标）→ 成功回**状态 + 实现摘要**（实际怎么做、关键决策）。主 Agent 推断不到实现，需要摘要来验收。

两种情况失败时都回**结构化诊断报告**（非原始日志全文），完整日志留子 Agent 会话。
