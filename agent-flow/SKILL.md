...
**【无判据 = 无借口】**
本规则**不设任何判据**（不看文件大小、不看操作次数、不看"是否需主 Agent 亲自判断"）。规则只有一条：**不得编写/执行，派子 Agent**。
任何"改动很小""文件很小""蓝图已精确""需要我亲自判断""临时加 log"**均不构成豁免理由**。

**【唯一解锁：§4.3 L3 接管】**
主 Agent 仅在触发 §4.3 L3 时可亲自编写/执行。L3 触发条件为**硬性客观条件**：
- L1 预算耗尽（cmd ≤3 次 / code-retriever ≤1 次 / code-executor ≤1 次），**且**
- L2 预算耗尽（general-purpose ≤5 次），
- 或趋势失控（错误雪崩 / 振荡 / 频发新报错）。

L3 三选项（从末版继续修 / 推倒重写 / 重新设计方案）此时允许主 Agent 亲自 Edit/Write/Bash。这是"子 Agent 都搞不定"的最后一道防线，触发条件是预算耗尽的客观事实，**不是主 Agent 主观判断，不构成豁免口子**。

**【4 项机制（保留，更新判定逻辑）】**

**机制 1 · 执行前显式判定（Pre-Execution Gate）**：开始任何 Edit/Write/Bash 前，主 Agent 必须显式声明——① 是否触发 L3（硬条件核对：L1 是否耗尽？L2 是否耗尽？趋势是否失控？）；② 若未触发 L3 → **禁止自做，必须派子 Agent**。"自行编辑/执行"从默认选项变为**仅 L3 可选项**。

**机制 2 · 调度强制（替代原硬阈值红线）**：编写/执行一律派子 Agent，**无判据、无规模阈值、无主观豁免**。这是比原"文件数 ≥3 或改动 ≥3 处"更严格的绝对禁令。

**机制 3 · 调度强制下限（保留）**：用户加载 `$agent-flow` 即视为期望调度行为。主 Agent 在一次任务中**必须至少调度一次子 Agent**（除非任务是纯决策 / 纯分析 / 纯对话 / 纯 TodoWrite）。全程零调度 = skill 加载失败，需在交付报告中说明原因。

**机制 4 · 交付报告调度审计（保留，审计项更新）**：任务完成的交付报告必须声明——① 本任务调度了哪些子 Agent（分别做什么）；② 主 Agent 是否在**非 L3 情况下**亲自编写/执行了 → 若有，**标记为违规需复盘**；③ 若触发了 L3，说明硬条件核对结果（L1 耗尽？L2 耗尽？趋势失控？）。

**【执行验证任务的特别说明】**
"写完后的 grep/wc 验证""跑测试看结果""看编译错误决定下一步"**均属执行类，禁主 Agent 自己跑**。即便是**决策依赖型执行**（需基于输出做架构判断），也**不得主 Agent 亲自 Bash**——而是在蓝图里要求 cmd-executor 回传"决策所需的关键行/错误摘要"，主 Agent 基于回传判断。
